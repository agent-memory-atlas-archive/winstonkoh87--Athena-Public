...
def sync_file_to_supabase(
    ...
    # Split content into breadcrumb-enriched chunks (TD-052 / T-20260919-01).
    # Each chunk is prepended with a hierarchical heading breadcrumb so that
    # chunks past index 0 retain document provenance in vector space.
    from athena.memory.contextual_chunker import contextual_chunk_text

    chunks = contextual_chunk_text(content, abs_file.name, chunk_size=4000, overlap=400)
    if not chunks:
        chunks = [content]
